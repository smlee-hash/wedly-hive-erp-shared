// FIELD_OPTIONS 같은 도메인 옵션 데이터는 보관함에 두지 않음. 각 앱의 _components/options.ts 에 정의.
// 도구 함수(getFieldOptions, addCustomOption 등)는 외부에서 FIELD_OPTIONS 를 인자로 받거나
// 사용자 추가 옵션만 다루도록 분리되어야 함. 현 단계에서는 함수가 자체 변수를 두는 형태로 유지.
